...
main ( int argc, char *argv[] )
{
  ...
  argStruct.testList = "UnidirAsync, BidirAsync";
...
runTest ( TESTPARAMS * testParams )
{
  ...
  for ( procIdx = 0; procIdx < argStruct.procListSize; procIdx++ )
  {
    ...
    for ( messIdx = 0; messIdx < messListSize; messIdx++ )
    {
      ...
      //  Bandwidth measurements for 0-sized messages will not provide useful results
      if ( testParams->id != LATEN && dp.msize <= 0)
          continue;
                  ...
                  int iters, double *result )
{
  ...
  else
  {
    ...
    currBW = ( aggCommSize / ( double ) maxTime ) / GB_SIZE;
  }
...
printParameters (  )
{
  ...
  if ( strcmp ( argStruct.testList, "Latency" ) != 0 )
    printf ( "  GB size for BW calculation           : %d\n", GB_SIZE );
...
processArgs ( int argc, char **argv )
{
          ...
          && dogetopt == 1 )
  {
    ...
    case 'w':
      ...
      for(i=0; i<TYPETOT; i++)
      {
         if(strstr(optarg, testParams[i].name) == NULL)
            count++;
      }
      if(count == TYPETOT)
      {
         prestaRankPrint(0, "%s is not a valid test name\n", optarg);
         printTestNames();
         exit(EXIT_FAILURE);
      }
